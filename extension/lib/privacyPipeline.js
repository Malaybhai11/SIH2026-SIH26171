...
export function rehydrateAction(action, vault, securityContext = {}) {
  ...
  const extractTokens = (str) => {
    ...
    const tokens = [];
    for (const m of str.matchAll(/\[([A-Z_]+)_\d+\]/g)) {
      tokens.push({
        token: m[0],
    ...
    if (vault?.labels) {
      for (const [surr, label] of vault.labels.entries()) {
        if (str.includes(surr) && !tokens.some((t) => t.token === surr)) {
          tokens.push({
            token: surr,
            type: label.slice(0, label.lastIndexOf("_")),
          });
        }
      }
    }
  ...
  // If no destinationOrigin is provided (e.g. non-browser context or unit test without origin), resolve directly
  if (!securityContext?.destinationOrigin) {
    ...
    a.__securityDecision = {
      allowed: true,
      decision: "ALLOW",
      reason: "No destination origin specified; resolving directly",
    };
    return a;
  }
