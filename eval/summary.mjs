...
const [pii, faces, red, screens, lat, benchmark, redteam] = await Promise.all(
  ["pii.json", "faces.json", "redaction.json", "screens.json", "latency.json", "benchmark.json", "redteam.json"].map(j),
);
...
const s = {
  ...
  redTeam_evasionResistance: redteam && { aggregate: redteam.aggregate, byTechnique: Object.fromEntries(Object.entries(redteam.byTechnique).map(([k, v]) => [k, { caught: v.caught, total: v.total }])), promptInjectionBaseline: redteam.promptInjectionBaseline },
  ...
  realSiteGeneralization: benchmark && { passRate: benchmark.passRate, passed: benchmark.passed, total: benchmark.total, sites: benchmark.sites, medianWallMs: benchmark.medianWallMs, protocol: benchmark.protocol },
};
...
| | Real-site task benchmark (E1) — never used to build/tune this agent | **${benchmark ? `${benchmark.passed}/${benchmark.total} (${pctf(benchmark.passRate)})` : "n/a"}** across ${benchmark?.sites.length ?? "?"} public sites, median ${benchmark ? (benchmark.medianWallMs / 1000).toFixed(1) : "?"}s/task |
...
| | Red-team (E2) — evasion techniques not defeated by formatting tricks | ${redteam ? `${redteam.aggregate.caught}/${redteam.aggregate.elements} across 6 techniques (2 known, scoped gaps: SVG-embedded text, canvas-bitmap text)` : "n/a"} |
